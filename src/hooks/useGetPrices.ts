...
export function useGetPrices(token0?: TokenItem, token1?: TokenItem): UseQueryResult<{ priceToken0: number, priceToken1: number } | undefined, Error | null> {
    return useQuery({
      queryKey: ["price-tokens", token0, token1],
      queryFn: async (): Promise<{ priceToken0: number, priceToken1: number }> => {
