...
export const MergePriceChart = ({
  chartId,
  tokenName,
}: MergePriceChartProps) => {
  const { data, isLoading } = useQuery({
    queryKey: ["merge-price-chart", chartId],
    staleTime: 1000 * 60 * 5,
    queryFn: async () => {
      const response = await axios.get<PriceResponse[]>(
        `${API_BASE_URL}/merge/${chartId}`,
        {
          params: {
            timeframe: "daily",
          },
      ...
      <AreaChart data={data}>
        ...
        <Tooltip
          ...
          formatter={(value, name) => {
            return [
              `$${formatNumber(value as number, { decimalDigits: 7 })}`,
              name === "wewePrice" ? "WEWE Price" : `${tokenName} Price`,
            ];
          }}
