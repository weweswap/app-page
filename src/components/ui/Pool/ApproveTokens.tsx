...
import {
  useDualDeposit,
  useEstimateMintShares,
} from "~/hooks/useDepositWewePool";
...
const ApproveTokens = ({
  ...
  const { data: estimationMintShares } = useEstimateMintShares(
    ...
    ethers
      .parseUnits(
        String(data?.amountToken0 || 0),
        selectedPool?.token0?.decimals || 18
      )
      .toString(),
    ethers
      .parseUnits(
        String(data?.amountToken1 || 0),
    ...
    async function deposit() {
      if (selectedPool && estimationMintShares && address) {
        await approveToken0(
          ...
          estimationMintShares.amount0
        );
        await approveToken1(
          selectedPool.token1.address,
          selectedPool.address,
          estimationMintShares.amount1
        );
        const txReceipt = await dualDeposit(
          selectedPool.address,
          estimationMintShares.mintAmount,
          address
        );
        ...
        const getUsdFees = async () => {
          const finalUsdValue =
            totalFee > 0n ? await usdConverter(totalFee) : 0;
  ...
  useEffect(() => {
    ...
    isErrorApproveToken0,
    isErrorApproveToken1,
    isErrorDualDeposit,
    hashApproveToken0,
    hashApproveToken1,
    hashDualDeposit,
  ...
  const finishSuccessfully =
    ...
    hashApproveToken0 &&
    hashApproveToken1 &&
    (!isPendingApproveToken0 ||
      !isPendingApproveToken1 ||
      !isPendingDualDeposit) &&
    (!isConfirmingApproveToken0 ||
      !isConfirmingApproveToken1 ||
      !isConfirmingDualDeposit);
          ...
          <div className="flex flex-col gap-2 text-right">
            <div className="flex items-center gap-2">
              <Typography className="text_light_gray" size="xs">
                {parseFloat(Number(data?.amountToken0).toFixed(2))}
              </Typography>
              <Image
              ...
              />
              <Typography className="text_light_gray" size="xs">
                {parseFloat(Number(data?.amountToken1).toFixed(2))}
              </Typography>
              <Image
          ...
          <div className="flex flex-col text-right">
            <div className="flex items-center gap-2">
              <Typography className="text_light_gray" size="xs">
                {ethers
                  .formatUnits(
                    estimationMintShares?.mintAmount?.toString() || "0"
                  )
                  .toString()}{" "}
                SHARES
              </Typography>
        ...
        <div className="flex items-center gap-3">
          ...
          !hashApproveToken0 ? (
            <>
              <Loader color="grey" />
              <Typography>
                Please Approve {selectedPool?.token0.symbol}
              </Typography>
            </>
          ) : (
            <>
        ...
        <div className="flex items-center gap-3">
          ...
          !hashApproveToken1 ? (
            <>
              <Loader color="grey" />
              <Typography>
                Please Approve {selectedPool?.token1.symbol}
              </Typography>
            </>
          ) : (
            <>
      ...
      {finishSuccessfully && (
        <div className="flex w-full flex-col gap-4">
          ...
          <Button
            ...
            onClick={() => handleDetails(hashDualDeposit!)}
          >
            <Typography secondary size="xs" fw={700} tt="uppercase">
              VIEW DETAILS
            </Typography>
