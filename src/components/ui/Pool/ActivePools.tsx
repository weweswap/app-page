    ...
    <>
      {!showDetails ? <Card className='overflow-x-scroll'>
        ...
        <table className="w-[fit-content] min-w-[100%] table-auto text-left bg_dark mt-5">
          ...
          <tbody>
            {DUMMY_TABLE_CONTENT.map(({ poolType, logo, type, pool, tvl, range, volume, apr }, index) => (
              <>
                ...
                <tr onClick={() => handleShowDetails({ poolType, logo, type, pool, tvl, volume, apr })} key={pool} className="bg-[#1c1c1c] w-[full] cursor-pointer hover:bg-[#202020]" style={{ borderBottom: '1rem solid black' }}>
                  <td className="p-4 font-bold ">
                    <div className='flex items-center gap-2'>
                      ...
                      <Typography size='xs' opacity={0.7}>{type}</Typography>
                    </div>
                  </td>
                  <td className="p-4">
                    <Typography size='xs' opacity={0.7}>{tvl}</Typography>
                  </td>
                  <td className="p-4">
                    <Typography size='xs' opacity={0.7}>{range}</Typography>
                  </td>
                  <td className="p-4">
                    <Typography size='xs' opacity={0.7}>{volume}</Typography>
                  </td>
                  <td className="p-4">
                    <Typography size='xs' opacity={0.7}>{apr}</Typography>
                  </td>
